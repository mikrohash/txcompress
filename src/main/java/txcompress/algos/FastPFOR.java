...
public class FastPFOR extends CompressionAlgo {
    ...
    protected CompressionResult compress() {
        ...
        byte[] bytes = getBytesOfTryteString();
        ...
        return new CompressionResult(integersToBytes(compressed));

    }

    @Override
    protected DecompressionResult decompress(CompressionResult compressionResult) {
        int[] recov = iic.uncompress(bytesToInt(compressionResult.getBytes()));
        byte[] bytes = integersToBytes(recov);
        // remove last 2 bytes
        bytes = Arrays.copyOf(bytes, bytes.length - 2);
        return new DecompressionResult(bytes);
    }
